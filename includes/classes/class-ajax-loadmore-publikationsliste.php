...
class AJAX_LoadMore_Publikationsliste extends AJAX_LoadMore
{
    ...
    {
        $part = mdb_ajax__publication__build_citation( $post->ID, MDB_BUILD_ARRAY );

        // Create replacement terms
        $replacements = array(
            '_PUBTITLE_'     => $part[0],
            '_PUBCITE_'      => $part[1],
            '_SHOW_DETAILS_' => __( 'Zeige Details', 'mdb_ajax' ),
            '_DETAILS_'      => __( 'Details', 'mdb_ajax' ),
            '_ID_'           => $post->ID,
...
add_action( 'wp_ajax_publikationsliste', array( 'AJAX_LoadMore_Publikationsliste', 'handle_AJAX' ) );
add_action( 'wp_ajax_nopriv_publikationsliste', array( 'AJAX_LoadMore_Publikationsliste', 'handle_AJAX' ) );
